...
if __name__ == "__main__":
    ...
    parser.add_option('--nr1' ,action='store',type='int',dest='NR1'   ,default=1, help='order of rho polynomial for generating model 1')
    parser.add_option('--np1' ,action='store',type='int',dest='NP1'   ,default=1, help='order of pt polynomial for generating model 1')
    parser.add_option('--nr2' ,action='store',type='int',dest='NR2'   ,default=2, help='order of rho polynomial for fitting model 2')
    parser.add_option('--np2' ,action='store',type='int',dest='NP2'   ,default=1, help='order of pt polynomial for fitting model 2')
    ...
    exec_me('python limit.py -M Bias --datacard %s/card_rhalphabet_muonCR_r%ip%i.txt --datacard-alt %s/card_rhalphabet_muonCR_r%ip%i.txt -o %s -t %i --lumi %f %s -r %s --rMin %s --rMax %s --mass %s'%(sigDir1, options.NR1, options.NP1, sigDir2, options.NR2, options.NP2, sigDir1, options.toys, options.lumi, dataString, options.r, options.rMin, options.rMax, options.mass),options.dryRun)
