...
def writeDataCard(boxes,txtfileName,sigs,bkgs,histoDict,histoDictSignal,tfile_signal,options,numberOfGenPtBins=0):
    obsRate = {}
    for box in boxes:
		    	...
		    	print proc, box
                	...
                	if proc=='wqq' or proc=='zqq' or 'hqq' in proc:
                    	...
                    	if rate>0:
                            ...
                            jesErrs['%s_GenpT%s_%s'%(proc,str(k),box)] =  1.0+(abs(rateJESUp-rate)+abs(rateJESDown-rate))/(2.*rate)
                            jerErrs['%s_GenpT%s_%s'%(proc,str(k),box)] =  1.0+(abs(rateJERUp-rate)+abs(rateJERDown-rate))/(2.*rate)
                            puErrs['%s_GenpT%s_%s'%(proc,str(k),box)] =  1.0+(abs(ratePuUp-rate)+abs(ratePuDown-rate))/(2.*rate)
                    	else:
                            jesErrs['%s_GenpT%s_%s'%(proc,str(k),box)] =  1.0
                            jerErrs['%s_GenpT%s_%s'%(proc,str(k),box)] =  1.0
    ...
    for proc in sigs+bkgs:
	    ...
	    elif tfile_signal is not None and 'hqq' in proc:
		for k in range(1, numberOfGenPtBins + 1):
...
def main(options, args):
    ...
    if options.ifile_signal is not None:
        ...
        numberOfGenPtBins = histo_temp.GetYaxis().GetNbins()
	...
	for box in boxes:
		...
		histoDict['%s_%s'%(proc,box)] = tfile.Get('%s_%s'%(proc,box)).Clone()
	        histoDict['%s_%s'%(proc,box)].Scale(GetSF(proc,box,tfile))
		print "%s_%s", '%s_%s'%(proc,box)
		print "integral: ", histoDict['%s_%s'%(proc,box)].Integral()
                for syst in systs:
                    print 'getting histogram for process: %s_%s_%sUp'%(proc,box,syst)
                    histoDict['%s_%s_%sUp'%(proc,box,syst)] = tfile.Get('%s_%s_%sUp'%(proc,box,syst)).Clone()
                    histoDict['%s_%s_%sUp'%(proc,box,syst)].Scale(GetSF(proc,box,tfile))
	...
	for proc in sigs:
		...
		for k in range(1, numberOfGenPtBins + 1):
		    ...
		    print "name: ", name
                    if name == 'vbfhqq125_GenpT1_fail' or name == 'vbfhqq125_GenpT2_fail' or name == 'vbfhqq125_GenpT3_fail' or name == 'vbfhqq125_GenpT4_fail' or name == 'vbfhqq125_GenpT5_fail' or name == 'vbfhqq125_GenpT6_fail' or name == 'vbfhqq125_GenpT7_fail' or name == 'vbfhqq125_GenpT8_fail':
 	               entries +=  histoDictSignal_1D['%s_GenpT%s_%s'%(proc,str(k),box)].Integral()
    ...
    writeDataCard(boxes,txtfileName,sigs,bkgs,histoDict,histoDictSignal_1D,tfile_signal,options,numberOfGenPtBins)
    print '\ndatacard:\n'
