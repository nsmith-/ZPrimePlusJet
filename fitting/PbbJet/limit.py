...
def ftest(base,alt,ntoys,iLabel,options):
    if not options.justPlot:
        ...
        exec_me('combine -M GoodnessOfFit %s --rMax %s --rMin -%s -t %i --toysFile %s/higgsCombine%s.GenerateOnly.mH120.%s.root --algorithm saturated -n %s --freezeParameters %s' % (base,options.rMax,options.rMin,ntoys,options.odir,baseName,options.seed,baseName, options.freezeNuisances),options.dryRun)
        ...
        exec_me('combine -M GoodnessOfFit %s --rMax %s --rMin -%s -t %i --toysFile %s/higgsCombine%s.GenerateOnly.mH120.%s.root --algorithm saturated -n %s --freezeParameters %s' % (alt,options.rMax,options.rMin,ntoys,options.odir,baseName,options.seed,altName, options.freezeNuisances),options.dryRun)
        exec_me('cp higgsCombine%s.GoodnessOfFit.mH120.%s.root %s/toys2_%s.root'%(altName,options.seed,options.odir,options.seed))
