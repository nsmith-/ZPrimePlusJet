...
class dazsleRhalphabetBuilder:
    ...
    def LoopOverPtBins(self):
        ...
        for ipt in range(1, self._nptbins + 1):
            ...
            hpass_inPtBin = [];
            hfail_inPtBin = [];
            for h in self._hpass: hpass_inPtBin.append(
                proj("cat", str(ipt), h, self._mass_nbins, self._mass_lo, self._mass_hi));
            for h in self._hfail: hfail_inPtBin.append(
                proj("cat", str(ipt), h, self._mass_nbins, self._mass_lo, self._mass_hi));
            ...
            lParHists = self.makeRhalph([hfail_inPtBin[0], hfail_inPtBin[1], hfail_inPtBin[2], hfail_inPtBin[4]], pPt,
                                        "cat" + str(ipt))
            ...
            # #Write to file
            self.makeWorkspace(self._outputName, [pDatas[0]], lPHists, self._allVars, "pass_cat" + str(ipt), True, True,
                               pPt)
            self.makeWorkspace(self._outputName, [pDatas[1]], lFHists, self._allVars, "fail_cat" + str(ipt), True, True,
                               pPt)
    ...
    def makeRhalph(self, iHs, iPt, iCat):
        ...
        for i0 in range(1, self._mass_nbins + 1):
            ...
            pFail = r.RooRealVar(lName + "_fail_" + iCat + "_Bin" + str(i0), lName + "_fail_" + iCat + "_Bin" + str(i0),
                                 pSum, 0, max(pSum + pUnc, 0))
            ...
            pPass = r.RooFormulaVar(lName + "_pass_" + iCat + "_Bin" + str(i0),
                                    lName + "_pass_" + iCat + "_Bin" + str(i0), "@0*max(@1,0)*@2", lArg)
            ...
            if pSum < 4:
                ...
                pPass = r.RooRealVar(lName + "_pass_" + iCat + "_Bin" + str(i0),
                                     lName + "_pass_" + iCat + "_Bin" + str(i0), 0, 0, 0)
    ...
    def workspaceInputs(self, iHP, iHF, iBin, iPt):
        ...
        lData = r.RooDataHist("comb_data_obs", "comb_data_obs", r.RooArgList(self._lMSD), r.RooFit.Index(lCats),
                              r.RooFit.Import("pass", lPData), r.RooFit.Import("fail", lFData))
    ...
    def rooTheHistFunc(self, iH, iLabel="w", iBin="_cat0"):
        ...
        # normalization
        lNTot = r.RooRealVar(iLabel + "norm" + iBin, iLabel + "norm" + iBin, (iH[0].Integral() + iH[1].Integral()), 0.,
                             5. * (iH[0].Integral() + iH[1].Integral()))
        lNPass = r.RooFormulaVar(iLabel + "fpass" + iBin, iLabel + "norm" + iBin + "*(veff)",
                                 r.RooArgList(lNTot, self._lEff))
        lNFail = r.RooFormulaVar(iLabel + "fqail" + iBin, iLabel + "norm" + iBin + "*(1-veff)",
                                 r.RooArgList(lNTot, self._lEff))
        # shapes
        ...
        lP = r.RooHistPdf(iLabel + "passh" + iBin, iLabel + "passh" + iBin, r.RooArgList(self._lShift),
                          r.RooArgList(self._lMSD), lPData, 0)
        lF = r.RooHistPdf(iLabel + "failh" + iBin, iLabel + "failh" + iBin, r.RooArgList(self._lShift),
                          r.RooArgList(self._lMSD), lMData, 0)
    ...
    def makeWorkspace(self, iOutput, iDatas, iFuncs, iVars, iCat="cat0", iShift=True, iSyst=True, ptVal=500):
        ...
        for pFunc in iFuncs:
            ...
            if iSyst and ('tqq' in process or 'wqq' in process or 'zqq' in process or 'hqq' in process):
                ...
                for syst in systematics:
                    if syst == 'mcstat':
                        ...
                        tmph_mass_down = proj('cat', str(ipt), tmph_down, self._mass_nbins, self._mass_lo,
                                              self._mass_hi)
                        ...
                        histDict[
                            pFunc.GetName() + '_' + pFunc.GetName().replace('_', '') + syst + 'Down'] = tmph_mass_down
                        if 'tqq' in process:
                ...
                for h in hout:
                    for i in range(1, h.GetNbinsX() + 1):
                        ...
                        if rhoVal < RHO_LO or rhoVal > RHO_HI:
                            ...
                            rhoVal, h.GetName(), ptVal, h.GetXaxis().GetBinLowEdge(i), h.GetXaxis().GetBinUpEdge(i))
                            h.SetBinContent(i, 0.)
                            h.SetBinError(i, 0.)
            ...
            if iShift and ('wqq' in process or 'zqq' in process or 'hqq' in process):
                ...
                tmph_mass_unmatched = proj('cat', str(ipt), tmph_unmatched, self._mass_nbins, self._mass_lo,
                                           self._mass_hi)
...
def loadHistograms(f, pseudo, blind, useQCD, scale, rSignal):
    hpass = []
    hfail = []
            ...
            if useQCD:
                ...
                for i in range(1, qcd_pass_real.GetNbinsX() + 1):
                    for j in range(1, qcd_pass_real.GetNbinsY() + 1):
                        if qcd_pass_real.GetXaxis().GetBinCenter(i) > MASS_LO and qcd_pass_real.GetXaxis().GetBinCenter(
                                i) < MASS_HI:
    ...
    if pseudo:
        ...
        for i, sig in enumerate(sigs):
            hpass[0].Add(hpass_sig[i],rSignal)
            hfail[0].Add(hfail_sig[i],rSignal)
        ...
        for i in range(1, lH.GetNbinsX() + 1):
            for j in range(1, lH.GetNbinsY() + 1):
                ...
                if blind and massVal > BLIND_LO and massVal < BLIND_HI:
                    print "blinding signal region for %s, mass bin [%i,%i]" % (
                    lH.GetName(), lH.GetXaxis().GetBinLowEdge(i), lH.GetXaxis().GetBinUpEdge(i))
                    lH.SetBinContent(i, j, 0.)
                if rhoVal < RHO_LO or rhoVal > RHO_HI:
                    print "removing rho = %.2f for %s, pt bin [%i, %i], mass bin [%i,%i]" % (
                    rhoVal, lH.GetName(), lH.GetYaxis().GetBinLowEdge(j), lH.GetYaxis().GetBinUpEdge(j),
                    lH.GetXaxis().GetBinLowEdge(i), lH.GetXaxis().GetBinUpEdge(i))
                    lH.SetBinContent(i, j, 0.)
        lH.SetDirectory(0)
