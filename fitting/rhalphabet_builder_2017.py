...
class RhalphabetBuilder():
    ...
    def prefit(self):
        ...
        for cat in categories:
            ...
            norms_b.add(wralphabase[cat].function('qcd2017_%s_norm' % cat))
            norms_s.add(wralphabase[cat].function('qcd2017_%s_norm' % cat))
            pdfs_b = r.RooArgList()
            pdfs_s = r.RooArgList()
            #pdfs_b.add(wralphabase[cat].pdf('qcd_%s' % cat))
            #pdfs_s.add(wralphabase[cat].pdf('qcd_%s' % cat))
            pdfs_b.add(wralphabase[cat].pdf('qcd2017_%s' % cat))
            pdfs_s.add(wralphabase[cat].pdf('qcd2017_%s' % cat))

            data[cat] = wbase[cat].data('data_obs_%s' % cat)
            for proc in (bkgs + sigs):
                if proc == 'qcd' or proc=='qcd2017': continue
    ...
    def MakeRhalphabet(self, samples, fail_histograms, pt, category):
        ...
        self.buildPolynomialArray(polynomial_variables, self._poly_degree_pt, self._poly_degree_rho, "p2017", "r2017", -30, 30)
