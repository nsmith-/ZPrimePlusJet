...
def parser():
    ...
    parser.add_option('--toys'   ,action='store',type='int',dest='toys'   ,default=500,help='mass')
...
def setup(iLabel,mass,iBase,iAlt):
    ...
    os.system('sed "s@XXX@%s@g" card_1D_%s_tmp.txt > %s/card_1D_%s.txt' %(mass,iBase,iLabel,iBase))
    os.system('sed "s@XXX@%s@g" card_1D_%s_tmp.txt > %s/card_1D_%s.txt' %(mass,iAlt ,iLabel,iAlt))
...
if __name__ == "__main__":
    options = parser()
    print options
    setup('ZQQ_'+str(options.mass),options.mass,"bern3","bern4")
    limit('card_1D_bern3.txt')
    ftest('card_1D_bern3.txt','card_1D_bern4.txt',options.toys,"ftest"+str(options.mass))
    goodness('card_1D_bern3.txt',options.toys,"goodness"+str(options.mass))
    bias('card_1D_bern3.txt','card_1D_bern4.txt',options.toys,options.sig,"fitbias"+str(options.mass))
    bias('card_1D_bern3.txt','card_1D_bern3.txt',options.toys,options.sig,"fitbase"+str(options.mass))
    plotmass('card_1D_bern3.txt',options.mass)
