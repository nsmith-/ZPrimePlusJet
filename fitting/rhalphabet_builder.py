...
class RhalphabetBuilder():
    ...
    def prefit(self):
        ...
        for cat in categories:
            ...
            pdfs_s = r.RooArgList()
            if self._suffix:
                norms_b.add(wralphabase[cat].function('qcd_%s_%s_norm' %( cat,self._suffix)))
                norms_s.add(wralphabase[cat].function('qcd_%s_%s_norm' %( cat,self._suffix)))
                pdfs_b.add(wralphabase[cat].pdf('qcd_%s_%s'            %( cat,self._suffix)))
                pdfs_s.add(wralphabase[cat].pdf('qcd_%s_%s'            %( cat,self._suffix)))
            else:
    ...
    def MakeRhalphabet(self, samples, fail_histograms, pt, category):
        ...
        for mass_bin in range(1, self._mass_nbins + 1):
            ...
            if self._suffix:
                pass_bin_var = r.RooFormulaVar(rhalph_bkgd_name + "_pass_" + category +"_"+self._suffix+ "_Bin" + str(mass_bin),
                                               rhalph_bkgd_name + "_pass_" + category +"_"+self._suffix+ "_Bin" + str(mass_bin),
                                               "@0*max(@1,0)*@2", lArg)
            else:
        ...
        #    pass_bins[i].Print()
        if self._suffix:
            pass_rparh = r.RooParametricHist(rhalph_bkgd_name + "_pass_" + category+"_"+self._suffix, 
                                             rhalph_bkgd_name + "_pass_" + category+"_"+self._suffix,
                                             self._lMSD, pass_bins, fail_histograms["data_obs"])
            fail_rparh = r.RooParametricHist(rhalph_bkgd_name + "_fail_" + category+"_"+self._suffix,
                                             rhalph_bkgd_name + "_fail_" + category+"_"+self._suffix,
        ...
        fail_rparh.Print()
        if self._suffix:
            pass_norm = r.RooAddition(rhalph_bkgd_name + "_pass_" + category +"_"+self._suffix+ "_norm",
                                      rhalph_bkgd_name + "_pass_" + category +"_"+self._suffix+ "_norm", pass_bins)
            fail_norm = r.RooAddition(rhalph_bkgd_name + "_fail_" + category +"_"+self._suffix+ "_norm",
                                      rhalph_bkgd_name + "_fail_" + category +"_"+self._suffix+ "_norm", fail_bins)
        else:
        ...
        if self._suffix:
            getattr(pass_workspace, 'import')(pass_rparh, r.RooFit.RecycleConflictNodes(), r.RooFit.RenameAllVariablesExcept(self._suffix,'x'))
            getattr(pass_workspace, 'import')(pass_norm, r.RooFit.RecycleConflictNodes(), r.RooFit.RenameAllVariablesExcept( self._suffix,'x'))
            getattr(fail_workspace, 'import')(fail_rparh, r.RooFit.RecycleConflictNodes(), r.RooFit.RenameAllVariablesExcept(self._suffix,'x'))
            getattr(fail_workspace, 'import')(fail_norm, r.RooFit.RecycleConflictNodes(), r.RooFit.RenameAllVariablesExcept( self._suffix,'x'))
        else:
    ...
    def buildRooPolyArray(self, iPt, iMass, iQCD, iZero, iVars):
        ...
        for pRVar in range(0, self._poly_degree_rho + 1):
            ...
            for pVar in range(0, self._poly_degree_pt + 1):
                ...
                lNCount = lNCount + 1
            if self._suffix:
                pLabel = "Var_Pol_Bin_" + str(round(iPt, 2)) + "_" + str(round(iMass, 3)) + "_" + str(pRVar)+"_"+self._suffix
            else:
        ...
        if self._suffix:
            lLabel = "Var_MassPol_Bin_" + str(round(iPt, 2)) + "_" + str(round(iMass, 3)) +"_"+self._suffix
        else:
    ...
    def buildRooPolyRhoArray(self, iPt, iRho, iQCD, iZero, iVars):
        ...
        for pRVar in range(0, self._poly_degree_rho + 1):
            ...
            for pVar in range(0, self._poly_degree_pt + 1):
                ...
                lNCount = lNCount + 1
            if self._suffix:
                pLabel = "Var_Pol_Bin_" + str(round(iPt, 2)) + "_" + str(round(iRho, 3)) + "_" + str(pRVar)+"_"+self._suffix
            else:
        ...
        if self._suffix:
            lLabel = "Var_RhoPol_Bin_" + str(round(iPt, 2)) + "_" + str(round(iRho, 3)) + "_"+self._suffix
        else:
