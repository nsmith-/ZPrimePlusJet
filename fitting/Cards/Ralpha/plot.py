...
parser = OptionParser()
parser.add_option('--input'  ,action='store',type='string',dest='input'    ,default='/afs/cern.ch/user/p/pharris/pharris/public/bacon/prod/CMSSW_7_1_20/src/VQQ/clean/Cat2D/mlfit.root',help='input file')
parser.add_option('--data'   ,action='store',type='string',dest='data'     ,default='/afs/cern.ch/user/p/pharris/pharris/public/bacon/prod/CMSSW_7_1_20/src/VQQ/clean/Cat2D/simple-shapes-RooDataHist.root',help='data file')
...
parser.add_option('--noewk'   ,action='store_true',         dest='noewk'   ,default=False,help='no V in fail') 
...
def draw(iData,iHists,iName="A",iPF=True,iDivide=False):
    ...
    for pHist in iHists:
        if pHist.GetName().find("fail") > -1:
            print "Drawing",pHist,pDraw,pHist.Integral()
            pHist.Draw("hist sames") if pDraw else pHist.Draw("e2 sames")
            pDraw=True
...
def loadData(iDataFile,iCat):
    ...
    #lW = iDataFile.Get("w_"+iCat.replace("pass_","").replace("fail_",""))
...
if __name__ == "__main__":
    ...
    for cat in options.cats.split(','):
        ...
        if options.passfail:
            ...
            lHists.extend(loadHist(lHFile,"fail_cat"+cat,not options.noewk,True))
        if len(lSum) == 0:
            lDSum = lData
            lSum  = lHists
        else:
