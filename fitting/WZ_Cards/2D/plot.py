...
parser = OptionParser()
parser.add_option('--input'  ,action='store',type='string',dest='input'    ,default='/afs/cern.ch/user/p/pharris/pharris/public/bacon/prod/CMSSW_7_1_20/src/VQQ/clean/2D/mlfit.root',help='input file')
parser.add_option('--data'   ,action='store',type='string',dest='data'     ,default='/afs/cern.ch/user/p/pharris/pharris/public/bacon/prod/CMSSW_7_1_20/src/VQQ/clean/2D/simple-shapes-RooDataHist.root',help='data file')
parser.add_option('--cats'    ,action='store',type='string',dest='cats'    ,default='1,2',help='categories 0,1,2,...')
...
if __name__ == "__main__":
    ...
    for cat in options.cats.split(','):
        ...
        if options.passfail:
            ...
            lHists.extend(loadHist(lHFile,"fail_cat"+cat,True,True))
        if len(lSum) == 0:
            lDSum = lData
            lSum  = lHists
        else:
