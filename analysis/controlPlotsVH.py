...
def makePlots(plot,hs,hb,hd,hall,legname,color,style,isData,odir,lumi,ofile,canvases):
    if isData:
        ...
        c = makeCanvasComparisonStack(hs,hb,legname,color,style,'WHbb',plot.replace('h_','stack_'),odir,lumi,True,ofile)
...
def main(options,args,outputExists):
    ...
    if not outputExists: 
        ...
        bkgSamples['QCD'] = sampleContainer('QCD',tfiles['QCD'], 1000, DBTMIN,lumi)
        if muonCR:
	    ...
	    bkgSamples['TTbar1Lep']  = sampleContainer('TTbar1Lep',tfiles['TTbar'], 1, DBTMIN,lumi, False, False, 'genMuFromW+genEleFromW+genTauFromW==1') 
	...
	sigSamples['WHbb'] = sampleContainer('WHbb',tfiles['WHbb'], 1, DBTMIN,lumi )
        sigSamples['ZHbb'] = sampleContainer('ZHbb',tfiles['ZHbb'], 1, DBTMIN,lumi )
