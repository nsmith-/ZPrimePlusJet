...
def makeCanvasComparison(hs,legname,color,style,name,pdir="plots",lumi=30,ofile=None,unitnorm=False):
    ...
    for iname, h in sorted(hs.iteritems(),key=lambda (k,v): v.Integral()):
        print iname+": here       %.4f "%(h.Integral())
    ...
    tag3 = ROOT.TLatex(0.18,0.79,"Simulation Preliminary")
...
def makeCanvasComparisonStack(hs,hb,legname,color,style,nameS,outname,pdir="plots",lumi=30,printSB=True,ofile=None):
   ...
   # 	print name
    ...
    for name, h in sorted(hs.iteritems(),key=lambda (k,v): -v.Integral()):
        if 'WH' in name: leg.AddEntry(h,legname[name],"l")
    ...
    for name, h in sorted(hb.iteritems(),key=lambda (k,v): v.Integral()):
        if ntotal>0:        
            ...
            i+=1
            print "H:        %.2f             : %.3f "%(nsig,nsig/ntotal*100.)
...
def makeCanvasComparisonStackWData(hd,hs,hb,legname,color,style,outname,pdir="plots",lumi=30,ofile=None,normalize=True,ratio=True):
    ...
    for name, h in sorted(hs.iteritems(),key=lambda (k,v): v.Integral()):
	if 'WH' in name or 'ttH' in name or 'VBF' in name: 
          h.SetLineColor(color[name])
          h.SetLineStyle(style[name])
          h.SetLineWidth(2)
          h.SetFillStyle(0)
    ...
    leg_y2 = 0.88 - (4+int(len(hb)/3))*0.03
    ...
    leg2.SetTextFont(42)
    leg3 = ROOT.TLegend(0.65,leg_y2,0.98,0.88,)
    leg3.SetFillStyle(0)
